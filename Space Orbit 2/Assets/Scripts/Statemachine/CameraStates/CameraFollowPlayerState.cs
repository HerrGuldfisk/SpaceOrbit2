...
public class CameraFollowPlayerState : BaseState
{
    ...
    {
        Camera.transform.position = Vector3.Lerp(_startPosition, LerpPosition(_transitionTime) , EaseInOutQuad(_transitionTime));
    ...
    private void FollowPlayer()
    {
        ...
        // Player.transform.position + (Vector3)Player.rb.velocity.normalized * Mathf.Pow(Player.currentSpeed + 1, 0.95f);
    }

    private Vector3 LerpPosition()
    {
        return Vector3.Lerp(Camera.transform.position, Player.transform.position + (Vector3)Player.rb.velocity.normalized * Mathf.Pow(Player.currentSpeed, 0.98f), Time.deltaTime * 2 );
    }
    private Vector3 LerpPosition(float transitionTime)
    {
        return Vector3.Lerp(Camera.transform.position, Player.transform.position + (Vector3)Player.rb.velocity.normalized * Mathf.Pow(Player.currentSpeed, 0.98f), 1 - transitionTime);
    }
